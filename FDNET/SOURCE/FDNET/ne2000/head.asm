...
if PKT_INTS	eq 2
	public	entry_point2
entry_point2	db	?,?,?,?		; interrupt to communicate.
endif
...
if PKT_INTS eq 2
	public	their_2nd_isr
their_2nd_isr	dd	0		; original owner of the 2'nd pkt driver int
endif
...
if PKT_INTS eq 2
	public	our_2nd_isr
our_2nd_isr:
	jmp	short our_isr_1		;the required signature.
	nop
	db	'PKT DRVR',0
endif
...
our_isr_0:
	...
	push	ax
if PKT_INTS eq 2
	extrn	which_int : byte, int_swap : byte
	mov		al, int_swap
	jmp		short	our_isr_2
our_isr_1:
	push	ax
...
default_handle:
	mov		al, driver_class
if PKT_INTS	eq 2
	cmp		which_int, 0	; Check if being called by the Ethernet int
	je		got_class
	mov		al, driver2_class		; No - get default class from second list
got_class:
endif
	mov		_CH[bp], al
got_handle:
...
if PKT_INTS eq 2
	; Setup the appropriate class list depending on which int vect was used
	cmp		byte ptr which_int, 0		; Check if called by the Ethernet int
	je		access_type_9
	mov		bx, offset driver2_class	; No - called by the IEEE802.5 int
endif
access_type_9:
...
if PKT_INTS eq 2
	mov	al,entry_point	; release our_2nd_isr.
	inc	al
	mov	ah,25h
	push	ds
	lds	dx,their_2nd_isr
	int	21h
	pop	ds
endif
...
;For backwards compatability, we special-base Ethernet II (BLUEBOOK) so that
;it matches both BLUEBOOK and IEEE8023.
	cmp	[bx].class,BLUEBOOK	;is this a BLUEBOOK handle?
	jne	recv_find_class		;no, match the class first.
	cmp	[bx].packet_type_len,0	;Are they asking for all of them?
	jne	recv_find_class		;no, match the class first.
	cmp	dl,BLUEBOOK		;is the frame BLUEBOOK?
	je	recv_find_3		;yes.
	cmp	dl,IEEE8023		;is the frame IEEE8023?
	je	recv_find_3		;yes.
recv_find_class:

	cmp	[bx].class, dl		;check the type in case it was BLUEBOOK.
...
;  to be filled by the driver.
	...
	jnc		recv_found
	call	recv_drop
	jmp		SHORT recv_find_5		; We didn't find it -- discard it.
;
recv_found:
