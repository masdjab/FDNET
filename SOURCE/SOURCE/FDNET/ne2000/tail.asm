...
if PKT_INTS	eq 2
	extrn	our_2nd_isr: near, their_2nd_isr: dword
	extrn	entry_point2: byte, int_swap : byte
endif
...
entry_point_name	db	"Packet driver Ethernet & IEEE802.3 software interrupt is ",'$'
entry_point2_name	db	"Packet driver IEEE802.5 software interrupt is ",'$'
endif
...
if PKT_INTS	eq 2
	cmp		al,'t'
	jne		not_t_opt
	mov		int_swap, 1			; Set the interrupt swap mode
	jmp		chk_options
not_t_opt:
endif
usage_error_j_1:
...
if PKT_INTS	eq 2
	mov		al, [di]			; Get the first entry point
	inc		al					; Second entry point is one larger than first
	mov		entry_point2, al	; Store the second entry point
endif
...
if PKT_INTS	eq 2
	mov	entry_point, 61h	; Defulat second entry point
endif
...
if PKT_INTS	eq	1
	...
	cmp		entry_point_fnd, 0		; Did we already find one?
	je		first_vect_found		; No - continue
	mov		al, entry_point_fnd		; Check if previously found int vect
	inc		al						;   is adjacent to the current vect.
	cmp		entry_point, al			; If adjacent then everything is ok
	je		terminate_check_int_1
	jne	terminate_check_int_2	;yes, it's ambiguous - give error.
first_vect_found:
endif
...
if PKT_INTS	eq	1
	...
	cmp		int_swap, 0
	je		no_swap1
	mov		dx, offset entry_point2_name
no_swap1:
	mov		di, offset entry_point
	call	print_number
	mov		dx, offset entry_point2_name
...
if PKT_INTS	eq 2
	mov		ah, 35h			; Remember their 2'nd packet interrupt.
	mov		al, entry_point2
	int		21h
	mov		their_2nd_isr.offs, bx
	mov		their_2nd_isr.segm, es

	mov		ah, 25h			; Install our 2'nd packet interrupt
	mov		dx, offset our_2nd_isr
	int		21h
endif
